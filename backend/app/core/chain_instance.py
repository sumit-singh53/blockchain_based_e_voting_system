...
_CHAIN_FILE = _backend_root / "chain.json"
_MEMPOOL_FILE = _backend_root / "mempool.json"
_storage = ChainStorage(path=str(_CHAIN_FILE), mempool_path=str(_MEMPOOL_FILE))
...
_loaded_mempool = _storage.load_mempool()
if _loaded_mempool:
    _chain.mempool = _loaded_mempool
...
def persist_chain() -> None:
    ...
    _storage.save_mempool(_chain.mempool)
